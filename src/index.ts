...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Wikifeed');
}
...
// Wikimedia's feed endpoints 404 unless month/day are zero-padded to
// two digits. Agents passing "5"/5/"May" or omitting padding got 404
// "Wikimedia error" responses — 9+ hits/24h in analytics. Normalize
// before building the URL: accept number or string, full month name,
// or already-padded value, and emit "01"-"12" / "01"-"31". Year is
// 4-digit normalized too (so "26" → "2026" works).
...
function padMonth(v: unknown): string {
  if (v == null || v === '') throw new Error('month is required (number 1-12 or name like "May" or padded "05")');
  if (typeof v === 'number') return String(v).padStart(2, '0');
  const s = String(v).trim().toLowerCase();
  if (MONTH_NAMES[s]) return MONTH_NAMES[s];
  const n = parseInt(s, 10);
  if (!Number.isFinite(n) || n < 1 || n > 12) throw new Error(`Invalid month "${v}" — expected 1-12 or a name like "May".`);
  return String(n).padStart(2, '0');
}

function padDay(v: unknown): string {
  if (v == null || v === '') throw new Error('day is required (1-31)');
  const n = typeof v === 'number' ? v : parseInt(String(v).trim(), 10);
  if (!Number.isFinite(n) || n < 1 || n > 31) throw new Error(`Invalid day "${v}" — expected 1-31.`);
  return String(n).padStart(2, '0');
}

function padYear(v: unknown): string {
  if (v == null || v === '') throw new Error('year is required (4-digit, e.g. "2026")');
  const n = typeof v === 'number' ? v : parseInt(String(v).trim(), 10);
  if (!Number.isFinite(n)) throw new Error(`Invalid year "${v}" — expected 4-digit number.`);
  // Accept 2-digit ("26") by mapping to 2000+, since Wikimedia only has
  // 2002-onward feed coverage anyway.
  return n < 100 ? String(2000 + n) : String(n);
}

async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  const month = args.month !== undefined ? padMonth(args.month) : undefined;
  const day = args.day !== undefined ? padDay(args.day) : undefined;
  const year = args.year !== undefined ? padYear(args.year) : undefined;
  switch (name) {
    case 'on_this_day':
      return onThisDay(month!, day!);
    case 'featured_article':
      return featuredArticle(year!, month!, day!);
    case 'most_read':
      return mostRead(year!, month!, day!);
    case 'picture_of_day':
      return pictureOfDay(year!, month!, day!);
    default:
...
async function onThisDay(month: string, day: string) {
  const res = await pwFetch(`${BASE_URL}/onthisday/all/${month}/${day}`, { headers: HEADERS });
  if (!res.ok) throw await httpError(res, 'Wikimedia error');
...
async function featuredArticle(year: string, month: string, day: string) {
  const res = await pwFetch(`${BASE_URL}/featured/${year}/${month}/${day}`, { headers: HEADERS });
  if (!res.ok) throw await httpError(res, 'Wikimedia error');
...
async function mostRead(year: string, month: string, day: string) {
  const res = await pwFetch(`${BASE_URL}/mostread/${year}/${month}/${day}`, { headers: HEADERS });
  if (!res.ok) throw await httpError(res, 'Wikimedia error');
...
async function pictureOfDay(year: string, month: string, day: string) {
  const res = await pwFetch(`${BASE_URL}/image/${year}/${month}/${day}`, { headers: HEADERS });
  if (!res.ok) throw await httpError(res, 'Wikimedia error');
